...
import { headId, jsonlLines, readJsonlArray } from "./jsonl";
...
describe("jsonl", () => {
	it("streams trimmed non-blank lines, the last one without a newline too", async () => {
		...
		expect(readJsonlArray<{ id: string }>(file).map((r) => r.id)).toEqual(["a", "b", "c"]);
	});

	it("throws on a missing file and peeks only a leading id", () => {
		expect(() => readJsonlArray(path.join(dir, "none.jsonl"))).toThrow("missing");
