...
import { readJsonlArray } from "../lib/jsonl";
...
function main(): void {
	...
	const full = new Set(
		fullFile ? readJsonlArray<Sample>(fullFile).map((s) => `${s.uuid}:${s.side}`) : []
	);

	const games = new Map<string, StoredGame>();
	for (const g of readJsonlArray<StoredGame>(gamesFile)) games.set(g.uuid, g);
	const samples = readJsonlArray<Sample>(samplesFile);
