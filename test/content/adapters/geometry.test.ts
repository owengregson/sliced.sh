...
describe("squareToPoint", () => {
	it("maps e2 on a white-bottom board and when flipped", () => {
		...
		expect(squareToPoint("e2", BOARD_RECT, false)).toEqual({ x: 397, y: 529 });
		// flipped: x = 100 + (7-4)*66 + 33 = 331; y = 100 + 1*66 + 33 = 199
		expect(squareToPoint("e2", BOARD_RECT, true)).toEqual({ x: 331, y: 199 });
		expect(squareToPoint("a1", BOARD_RECT, false)).toEqual({ x: 133, y: 595 });
		expect(squareToPoint("h8", BOARD_RECT, false)).toEqual({ x: 595, y: 133 });
	});
});
	...
	it("round-trips every square in both orientations", () => {
		for (const flipped of [false, true]) {
			for (const sq of ALL_SQUARES) {
				const p = squareToPoint(sq, BOARD_RECT, flipped);
