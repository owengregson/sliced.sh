...
describe("TimingModel.planMove", () => {
	it("thinkMs ≥ dragDurationMs, deadline = now + thinkMs, window sums to thinkMs, orientation present", () => {
		...
		for (let i = 0; i < 300; i++) {
			...
			if (plan.mode === "normal" || plan.mode === "long") {
				...
				expect(plan.window.decisionMs / plan.thinkMs).toBeGreaterThanOrEqual(0.15 - 1e-9);
				expect(plan.window.decisionMs / plan.thinkMs).toBeLessThanOrEqual(0.4 + 1e-9);
			}
	...
	it("untimed games skip pressure and caps and condition as classical", () => {
		...
		expect(plan.features.alloc).toBeCloseTo(45, 10);
	});
	...
	it("bot-pace floor keeps normal think times ≥ 0.6× the model median", () => {
		...
		let checked = 0;
		for (let i = 0; i < 400; i++) {
			const c = ctx({ oppThinkMsHistory: [400, 450, 420, 380] });
			const plan = m.planMove(c);
			if (plan.mode !== "normal") continue;
			const f = computeFeatures(c);
			const floor =
				0.6 * head.median(f, m.persona, plan.features.alloc ?? 0) * (plan.features.comp ?? 1);
			expect(plan.thinkMs / 1000).toBeGreaterThanOrEqual(floor - 1e-9);
			checked++;
		}
