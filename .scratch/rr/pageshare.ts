// R4.1's six cells, plan level, real ONNX. Runs unchanged on pre-lane, round 3 and tip.
...
const HIST = ["e2e4","e7e5","g1f3","b8c6","f1b5","a7a6","b5a4","g8f6","e1g1","f8e7","f1e1","b7b5","a4b3","d7d6","c2c3","e8g8","h2h3","c6b8","d2d4","b8d7"];
...
console.log("cell                              inbook inst%  prem%  sub2s%    p50");
for (const [lbl, baseSec, clockS, ply, chosen] of [
  ["10+0 480s ply16 offbook", 600, 480, 16, "a2a4"],
  ["10+0 480s ply16 topmove", 600, 480, 16, "d2d4"],
  ["10+0 480s ply8  inbook ", 600, 480, 8, "d2d4"],
  ["3+0  144s ply16 offbook", 180, 144, 16, "a2a4"],
  ["3+0  180s ply40 offbook", 180, 180, 40, "a2a4"],
  ["1+0   60s ply16 offbook", 60, 60, 16, "a2a4"],
] as const) {
  ...
  const m = new TimingModel(new ChessMimicHead({ infer: infer as never, fallback: new V1ParametricHead(), budgetMs: 600000 }), DEFAULT_SETTINGS.timing, createRng(`ps-${lbl}`));
  ...
  for (let i=0;i<N;i++){ if (i%40===0){ m.startGame({...meta, gameId:`g${i}`}); await m.prepare(c);} const plan=m.planMove(c);
    if (plan.mode==="instant") inst++; else if (plan.mode==="premove") prem++; if (plan.thinkMs<2000) sub2++; ts.push(plan.thinkMs); }
  console.log(`${lbl.padEnd(32)} ${String(f.in_book).padStart(4)} ${(100*inst/N).toFixed(1).padStart(6)} ${(100*prem/N).toFixed(1).padStart(6)} ${(100*sub2/N).toFixed(1).padStart(7)} ${pct(ts,0.5).toFixed(0).padStart(7)}`);
}
