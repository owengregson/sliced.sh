/** The `AutoQueue` (see `@service/auto-queue`). */
...
import { type Entry, entryFromRecord, firstWait, recordsOf } from "@service/auto-queue/entry";
import type { AutoQueueOptions, AutoQueueView } from "@service/auto-queue/types";
...
export class AutoQueue {
	...
	async schedule(
		...
		const rematch =
			this.options.rematch && rematchEligible(opponent, settings, session) && opponent
				? opponent.name
				: null;
		const { delay, status } = firstWait(session, rematch, now, this.options.rng);
		const entry: Entry = {
			gameId,
			dueAt: now + delay,
			attempts: 0,
		...
		while (!this.disposed)
			try {
				...
				for (const [key, record] of Object.entries(records)) {
					...
					const entry = entryFromRecord(record, record.dueAt, this.now);
					this.entries.set(tabId, entry);
					this.arm(tabId, entry);
				}
				this.changed();
	...
	private async changed(): Promise<void> {
		...
		if (!this.options.persistence) return;
		do {
			const records = recordsOf(this.tabIds(), this.entries, this.sessions);
			try {
