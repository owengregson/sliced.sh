...
| Entry | Parts | Shape |
|---|---|---|
| `service/game-session/session.ts` | `session/` | `GameSession` is the public face; `parts.ts` builds and wires the collaborators, `lifecycle.ts` starts and finishes games, `page-events.ts` and `position-arrival.ts` take what the content script sends, `assistant-switch.ts` the settings edges. `SessionCore` holds the shared per-game state and the common queries (may act, colour hold, target Elo, clocks, state machine). Collaborators own one concern each — position feed, move delivery, premove decision, queued premove, scramble hold, prediction, hand arming, resign flow, lobby hold, time control and re-plan, Maia warm-up, board marks, effects feed, executor binding, move recorder — and read `SessionCore` fields live, never a copy, because each game replaces them. `position-rules.ts` is the pure half; `reply-context.ts` holds the pure reads the pipeline input carries for the reply that arrived (the pondered answer, the executor's hover square). |
| `service/game-session/recommendation.ts` | `recommendation/` | `RecommendationPipeline.run()` is nine named stages: context → timing → policy → book → analysis → candidates → selection → plan → assembly. Budget, Maia-search and own-move rating rules are pure modules (`budget.ts`, `maia-search.ts`, `own-move.ts`). `tablebase.ts` starts the endgame tablebase probe beside the book lookup and, at selection, lets a ≤ 7-man tablebase answer replace the selector's move. `fast-reply.ts` is the fast-reply rule (a book answer or a pondered obvious recapture caps the own-move movetime); `timing.ts` is the timing stage: the context, the bounded inference with its candidate rows, the chosen move's row and the plan. |
...
| `core/strength/move-selector.ts` | `selector/`, `selector/strategies/` | `selectMove` is a pipeline over one `SelectionFrame`: prepare lines → resolve the Maia rating → mate guard → the first strategy that decides (full strength, Maia draw, native, sampled). Stage order is RNG draw order. Resolving the Maia rating applies the fitted `MAIA_CALIBRATION` (conditioning rating and temperature per time class) and hands the Maia draw the tempered policy (`MaiaRating.policy`). The heuristic prior is a rule table (`prior/rules.ts`). |
| `core/engine/brilliant.ts` | `brilliant/`, `brilliant/gates/` | The entry owns only the gate order: `evaluateBrilliant` returns the first failing gate's reason — chosen (`chosen.ts`) → gift (`gift.ts`) → well-formed evidence (`evidence.ts`) → holds (`holds.ts`) → victory lap (`victory-lap.ts`, with the quiet mating-threat proof in `mating-threat.ts`) — else `sound-sacrifice`. `planBrilliant` (`plan.ts`) is the engine-free half over `offers.ts` (offer detection and shapes), `material.ts` (move gain, the bounded exchange, `staticExchange`, balance) and `recovery.ts` (off-square and standing recoveries, the short checking-mate proof); `illusion.ts` holds the engine-line regain tests; `types.ts` the shared shapes. |
...
| `core/timing/timing-model.ts` | `timing-model/` | `planMove` = `normaliseSample` → `calibrateSample` → `guardPremove` → `composeThink` → `assemblePlan`; one `replan` function per reason. `state.ts` owns the per-game state and how a plan and an observation update it, `inference-gate.ts` when the head's inference is skipped. The v1/ChessMimic heads implement one `DistributionHead` strategy; `chessmimic-head.ts` → `chessmimic-head/` splits band selection, inputs, the budgeted port, the per-position row cache (`rows.ts`: the history-only row and one per timed move) and the decoding (`head.ts`). |
| `core/timing/calibration.ts` | `calibration/` | Reads the fitted think-time table (`TIMING_CALIBRATION`, `core/constants/timing-calibration.ts`): `table.ts` (time class, knot interpolation, the think shift), `premove.ts` (the calibrated premove attempt rates the session hands `premoveCandidate`), `situation.ts` (a move's situation and the obvious-recapture rule the fast reply shares). `timing-model/calibrate.ts` is the stage that applies it. |
...
| `core/motor/anticipation.ts` | `anticipation/` | Anticipatory hover: `reply.ts` (the reply the ponder's top line anticipates, the engage odds and the executor's once-per-turn `engagedAnticipation`), `hover.ts` (the `anticipate` spell over `SpellTimeline`, free movement only, and `withinHover`), `execution.ts` (the anticipated reply's physical latency, which `timing-model/compose.ts` plans with when `TimingContext.hoverSquare` is the moving piece). |
| `core/strength/premove.ts` | `premove/` | `premoveCandidate` owns the gate order (cheap gates → prediction → per-reply search → reason → Maia gate → attempt draws); `propensity.ts` the attempt rates (strength or calibrated, and the safe trade's thinning), `prediction.ts` the reply prediction and its confidence gate, `queue-safety.ts` the site-queue proofs, `gate.ts` the Maia gate. |
