...
| 3 | **Timing** | When the hand plays, given that it plays. | The user's four knobs — the `profile` preset row that headed this section was **removed on 2026-09-15** at the owner's request ("remove the timing presets 'fast natural slow' etc."), so the sliders always apply: `baseSpeed` (**Base speed**; `speedScale` until 2026-09-15, when it was inverted so higher means faster and given the hand's movement as well as the wait), `varianceScale`, `longThinkFrequency`, `premoveTendency` — the two that shape the think distribution together, the premove knob (an event, not a duration) last. |
...
| `automation.moveQualityChips` | ~~`boardEffects` off (new)~~ — **never, since 2026-09-15**: the owner made the two switches independent, and `moveQualityChipsFor` / `moveRatingSounds` / `forcedMateSounds` now hang off `moveQualityChips` alone |
