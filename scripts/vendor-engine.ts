...
export interface ThirdPartyNotice {
	...
	/** The Polyglot books in `BOOKS.dir` (Task 15). */
	bookFiles: VendoredFile[];
}

const LICHESS_DB = "https://database.lichess.org/";
const LICHESS_ELITE_DB = "https://database.nikonoel.fr/";
const BOOK_SCRIPT = "scripts/build-club-book.py";
...
Both Polyglot books are generated by \`${BOOK_SCRIPT}\` from games in the Lichess open database
(${LICHESS_DB}), which Lichess releases under the Creative Commons CC0 1.0 public-domain
dedication. Nothing under \`${n.registry.BOOKS.dir}\` is copied from a third-party book file.

| File | Games | Filter | Source months |
|---|---|---|---|
| \`${n.registry.BOOKS.gm2600}\` | Lichess Elite Database (the CC0 database filtered to 2400+ vs 2200+, no bullet; ${LICHESS_ELITE_DB}) | both players rated ≥ 2600, first 30 plies, ≥ 5 games per move, ≤ 360 KB | 2020-06, 2020-07, 2020-08 |
| \`${n.registry.BOOKS.club}\` | \`lichess_db_standard_rated_<month>.pgn.zst\` | both players rated 1200–1800, no bullet, first 30 plies, ≥ 15 games per move, ≤ 3 MB | 2013-01, 2013-02, 2013-03 |
...
export async function vendorEngine(): Promise<void> {
	...
	const bookFiles = await describe(path.join(ROOT, BOOKS.dir), [BOOKS.gm2600, BOOKS.club]);
	await writeFile(
		DOCS_DEST,
		renderThirdParty({ version, registry, engineFiles, typesFile, bookFiles })
	);
